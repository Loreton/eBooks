...
def calibre(v:SimpleNamespace, parser):
    ...
    excl_group.add_argument('--authors-from-ebooks',  action='store_true', help=f'{C.cyan}get authors from ebook in calibre library metadata {v.default}')
    excl_group.add_argument('--extract-text',    action='store_true', help=f'{C.cyan}extract text from epub files {v.default}')
    common_options(v=v, parser=subp, func_name=func_name)
