...
#
#################################
def FindAll(p, data, ignore_case: bool=True):
    ...
    occurrencies = p.findall(' '.join(data), flags=py_flags)
    if  occurrencies:
...
#          }
#################################
def FindIter(p, source_data: str, context_length: int=0, normalize_text: bool=False, ignore_case: bool=True):
    logger.info("processing: %s", p)
    occurrences = {}
    py_flags = re.UNICODE | re.IGNORECASE if ignore_case else re.UNICODE

    for match in p.finditer(source_data):
    # for match in re.finditer(p, source_data, flags=py_flags):
...
def AND_justCheck(source_data: str, string_list: list, word_boundary=False, normalize_text: bool=False, ignore_case: bool=True):
    ...
    source_data = ' '.join(source_data.split()) if normalize_text else source_data
    py_flags = re.UNICODE | re.IGNORECASE if ignore_case else re.UNICODE
...
#################################
@function_executing_time
def AND(source_data: str, string_list: list, word_boundary=False, context_length: int=0, normalize_text: bool=False, ignore_case: bool=True):
    '''
        Esegue una ricerca AND per verificare se tutte le parole di string_list sono presenti nel testo.
...
#################################
@function_executing_time
def multi_near_words(source_data: str, words_list: list, near: list, normalize_text: bool=False, ignore_case: bool=True):
