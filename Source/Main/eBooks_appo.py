...
class LnEBooks:
    ...
    def search_AND_words(self, fld_name, word, ignore_case=True):
        # '^(?=.*?\bone\b)(?=.*?\btwo\b)(?=.*?\bthree\b).*$'
        _p=r'^(?=.*?\b{0}\b)'.format(words[0])
        for word in words[1:]:
            _p+=r'(?=.*?\b{}\b)'.format(word)
        _p+=r'.*$'

        pattern=re.compile(_p, re.IGNORECASE)

    def search_OR_words(self, fld_name, word, ignore_case=True):
        # r'^.*?\b(one|two|three)\b.*$'
        _words='|'.join(words)
        pattern=re.compile(r'^.*?\b({0})\b.*$'.format(_words), re.IGNORECASE)
        sono qui
    ...
    # -    fld_name: campo dove effettuare la ricerca
    ####################################################
    def search_one_word(self, fld_name, word, ignore_case=True):
        pattern = re.compile(word, re.IGNORECASE)
        _filter = {'_id': word}
        rec = self._Dictionary.get_record(_filter)
        bookid_list = rec[fld]
        print('books for word:', word, ':',len(book_ids))
    ...
    # -  p=re.compile(r'\b{0}\W+(?:\w+\W+){2}?{1}\b'.format('degli', 'ospiti', '{0,5}'), re.IGNORECASE);p.findall(text)
    ####################################################
    def search_more_words(self, fld_name, words, ignore_case=True):
        pattern = re.compile(word, re.IGNORECASE)
        bookid_list=[]
        for word in words:
            _filter = {'_id': word}
        ...
        book_list = list(set(book_list)) # remove duplicates ... forse non serve
        return bookid_list
    ...
    # -  p=re.compile(r'\b{0}\W+(?:\w+\W+){2}?{1}\b'.format('degli', 'ospiti', '{0,5}'), re.IGNORECASE);p.findall(text)
    ####################################################
    def regex_near_search(self, fld_name, words, near, ignore_case=True):
        if len(words) == 1:
            _word1, = words
            # my_query = {fld_name: {"$regex": _word1, "$options" : "i" } }
            pattern = re.compile(_word1, re.IGNORECASE)
            # _filter = {'_id': _word1}
            # records = self._Dictionary.get_record(_filter)
            # print('books for word:', word, ':',len(rec[fld]))
            # book_list.extend(rec[fld])
            # records = self._Dictionary._collection[_word1]
            # print(records)
            # if _max <0: _max=_min
            # _minmax = '{0}{1},{2}{3}'.format('{', _min, _max, '}') # {m,M}
            # pattern=re.compile(r'\b{_word1}\W+(?:\w+\W+){_minmax}?{_word2}\b'.format(**locals()), re.IGNORECASE)

        if len(words) == 2 and not _min<0:
            _word1, _word2, = words
            if _max <0: _max=_min
            _minmax = '{0}{1},{2}{3}'.format('{', _min, _max, '}') # {m,M}
            pattern=re.compile(r'\b{_word1}\W+(?:\w+\W+){_minmax}?{_word2}\b'.format(**locals()), re.IGNORECASE)
    ...
    # -  p=re.compile(r'\b{0}\W+(?:\w+\W+){2}?{1}\b'.format('degli', 'ospiti', '{0,5}'), re.IGNORECASE);p.findall(text)
    ####################################################
    def regex_near_search_OK(self, fld_name, near, ignore_case=True):
        xx = near[0].split()
